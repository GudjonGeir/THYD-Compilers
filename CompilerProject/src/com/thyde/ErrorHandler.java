...
public class ErrorHandler {
    ...
    protected void printErrorInfo(String errorMessage, Token errorToken) {
        String srcLine = getLineFromSource(errorToken.getLineNumber());
        String lineNumString = String.valueOf(errorToken.getLineNumber());
        while (lineNumString.length() < 3)
            lineNumString = " " + lineNumString;

        System.out.println (lineNumString + ": " + srcLine);
        System.out.print("     ");
        for(int n=0;n<errorToken.getColumn();n++)
            System.out.print(" ");
        System.out.print("^ ");
